 ...
 * @version 1.0
 */
public class AsciiToTextFormatter extends FileFormatterBase implements FormatterConstant {
  private static final Logger _logger = LoggerFactory.getLogger(AsciiToTextFormatter.class);
    ...
    while (true) {
      ...
      if (row > 0) {
        ...
        for (int col = 0; col < v.size(); col++) {
          out.writeln("[" + row + "," + col + "] " + headings.get(col) + ": " + v.get(col));
        }
        out.writeln();
      }
      row++;
