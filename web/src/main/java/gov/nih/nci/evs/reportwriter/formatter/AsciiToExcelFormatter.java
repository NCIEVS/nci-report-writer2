  ...
  public Boolean convert(String textfile, String delimiter, String outfile) throws Exception {
    ...
    for (int i = 0; i < maxChars.size(); i++) {
      ...
      if (maxCellLen < MAX_CODE_WIDTH) {
        ...
        if (tokens.size() > heading_height_multiplier) heading_height_multiplier = tokens.size();
      }
    }
    ...
    while (true) {
      ...
      if (_specialCases_CDISC) {
        ...
        */
        highlight_row =
            extensible_col != -1 && v.elementAt(extensible_col - 1).trim().length() == 0;
      }
        ...
        if (_ncitCodeColumns.contains(i) && rownum > 0 && s != null && s.length() > 0) {
          try {
            ...
            */
            e.printStackTrace();
            System.out.print(
                "WARNING: The following string is too large to be a "
                    + "valid NCIt code ("
                    + filename
                    + "): "
                    + s);
