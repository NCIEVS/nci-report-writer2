...
public class ExcelWriter {
  ...
  public void write() {
    Vector colors = readFile("colors.txt");
  ...
  public String[] getColumnHeadings(String heading_line, char delim) {
    Vector v = parseData(heading_line, delim);
  ...
  public void writeToXSSF(
      Vector datafile_vec,
      String excelfile,
      char delim,
      Vector sheetLabel_vec,
      String headerColor) {
    ...
    for (int lcv = 0; lcv < datafile_vec.size(); lcv++) {
      ...
      Vector lines = readFile(datafile);
      ...
      for (int i = 1; i < lines.size(); i++) {
        ...
        Vector values = parseData(line, delim);
  ...
  public static String cloneWorkbook(String xlsxfile) {
    ...
    for (int i = 0; i < workbook.getNumberOfSheets(); i++) {
      ...
      Cell cell = null;
      System.out.println(
          "Number of rows in " + sheet.getSheetName() + ": " + sheet.getPhysicalNumberOfRows());
