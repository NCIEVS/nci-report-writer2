...
public class RWUtils {
   ...
   *
   * @param reportOutput ReportOutput contains the data retrieved from the SPARQL queries and
   *     processing.
   ...
   */
  public void processSubset(
      List<String> codes,
      Report reportOutput,
      HashMap<String, EvsConcept> conceptHash,
    ...
    for (int i = 0; i < codes.size(); i++) {
      ...
      EvsConcept assocConcept =
          sparqlQueryManagerService.getEvsConceptDetailShort(code, namedGraph, restURL);
      assocConcept.setProperties(
          sparqlQueryManagerService.getEvsProperties(code, namedGraph, restURL));
  ...
  public void processSubset(
      Report reportOutput,
      List<EvsConcept> associatedConcepts,
      HashMap<String, EvsConcept> conceptHash,
      List<TemplateColumn> templateColumns,
      PrintWriter logFile,
      String namedGraph,
    ...
    for (EvsConcept concept : associatedConcepts) {
      ...
      if (conceptHash.containsKey(concept.getCode())) {
        ...
        concept.setProperties(
            sparqlQueryManagerService.getEvsProperties(concept.getCode(), namedGraph, restURL));
        concept.setAxioms(
            sparqlQueryManagerService.getEvsAxioms(concept.getCode(), namedGraph, restURL));
        conceptHash.put(concept.getCode(), concept);
      }
      try {
        writeColumnData(
            reportOutput, null, concept, conceptHash, templateColumns, namedGraph, restURL);
      } catch (Exception ex) {
        logFile.println(
            "ERROR: writeColumnData failed at "
                + concept.getLabel()
                + " ("
                + concept.getCode()
                + ")");
  ...
  public void processConceptInSubset(
      Report reportOutput,
      EvsConcept rootConcept,
      HashMap<String, EvsConcept> conceptHash,
      List<TemplateColumn> templateColumns,
      PrintWriter logFile,
      String namedGraph,
      String restURL) {
    List<EvsConcept> associatedConcepts =
        sparqlQueryManagerService.getEvsConceptInSubset(rootConcept.getCode(), namedGraph, restURL);
    log.info(
        "Concept: "
            + rootConcept.getCode()
            + " Number of associations: "
            + associatedConcepts.size());
    System.out.println(
        "Concept: "
            + rootConcept.getCode()
            + " Number of associations: "
    ...
    for (EvsConcept concept : associatedConcepts) {
      ...
      try {
        writeColumnData(
            reportOutput, rootConcept, concept, conceptHash, templateColumns, namedGraph, restURL);
      } catch (Exception ex) {
        ex.printStackTrace();
      }
    }
  ...
  public void processConceptInSubset(
    ...
    if (currentLevel < maxLevel) {
      List<EvsConcept> associatedConcepts =
          sparqlQueryManagerService.getEvsConceptInSubset(
              rootConcept.getCode(), namedGraph, restURL);
      log.info(
          "Concept: "
              + rootConcept.getCode()
              + " Number of associations: "
   ...
   * @param conceptHash ConceptHash cache to improve performance.
   * @param templateColumns TemplateColumns contain template definitions for each column.
   *     <p>This methods supports recursion.
   */
  public void processConceptSubclasses(
      Report reportOutput,
      EvsConcept parentConcept,
      HashMap<String, EvsConcept> conceptHash,
      List<TemplateColumn> templateColumns,
      int currentLevel,
      int maxLevel,
    ...
    if (currentLevel < maxLevel) {
      ...
      List<EvsConcept> subclasses =
          sparqlQueryManagerService.getEvsSubclasses(parentConcept.getCode(), namedGraph, restURL);
      log.info(
          "Parent Concept: "
      ...
      for (EvsConcept subclass : subclasses) {
        if (conceptHash.containsKey(subclass.getCode())) {
          ...
          subclass.setProperties(
              sparqlQueryManagerService.getEvsProperties(subclass.getCode(), namedGraph, restURL));
          subclass.setAxioms(
              sparqlQueryManagerService.getEvsAxioms(subclass.getCode(), namedGraph, restURL));
          conceptHash.put(subclass.getCode(), subclass);
        }
        processConceptInSubset(
            reportOutput, subclass, conceptHash, templateColumns, logFile, namedGraph, restURL);
        processConceptSubclasses(
            reportOutput,
            subclass,
            conceptHash,
            templateColumns,
   ...
   * Run the ConceptSubclasses query and then process the report based on the report template.
   *
   * <p>This is a special case where the recursion limit is set by the maxLevel parameter.
   *
   * @param reportOutput ReportOutput contains the data retrieved from the SPARQL queries and
   *     processing.
   * @param rootConcept Root Concept.
   ...
   *     <p>This methods supports recursion, but limited by the maxLevel parameters.
   */
  public void processConceptSubclassesOnly(
      Report reportOutput,
      EvsConcept parentConcept,
      HashMap<String, EvsConcept> conceptHash,
      List<TemplateColumn> templateColumns,
  ...
  /**
   * Uses the conceptFile to locate individual concepts and then report on the concept using the
   * template definition.
   *
   * <p>The conceptFile is a text file containing one concept per line.
   *
   * @param reportOutput ReportOutput contains the data retrieved from the SPARQL queries and
   *     processing.
   * @param conceptHash ConceptHash cache to improve performance.
   ...
   */
  public void processConceptList(
      Report reportOutput,
      HashMap<String, EvsConcept> conceptHash,
      List<TemplateColumn> templateColumns,
      String conceptFile,
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        EvsConcept concept =
            sparqlQueryManagerService.getEvsConceptDetailShort(line, namedGraph, restURL);
        concept.setProperties(
            sparqlQueryManagerService.getEvsProperties(concept.getCode(), namedGraph, restURL));
        concept.setAxioms(
            sparqlQueryManagerService.getEvsAxioms(concept.getCode(), namedGraph, restURL));
        try {
          writeColumnData(
              reportOutput, concept, concept, conceptHash, templateColumns, namedGraph, restURL);
        } catch (Exception ex) {
          ex.printStackTrace();
        }
    ...
    try {
      for (TemplateColumn column : templateColumns) {
        ...
        if (label.compareTo("CDISC Submission Value") == 0) {
          ...
          List<EvsConcept> superclasses =
              sparqlQueryManagerService.getEvsSuperclasses(concept.getCode(), namedGraph, restURL);
          ...
          if (properties.size() > 0) {
            String code =
                properties
                    .get(0)
                    .replaceAll("http://ncicb.nci.nih.gov/xml/owl/EVS/Thesaurus.owl#", "");
            values.add(code);
          }
          ...
          if (properties.size() > 0) {
            ...
            if (conceptHash.containsKey(code)) {
              ...
              nichdParentConcept =
                  sparqlQueryManagerService.getEvsConceptDetailShort(code, namedGraph, restURL);
              nichdParentConcept.setProperties(
                  sparqlQueryManagerService.getEvsProperties(code, namedGraph, restURL));
              nichdParentConcept.setAxioms(
                  sparqlQueryManagerService.getEvsAxioms(code, namedGraph, restURL));
          ...
          for (EvsAxiom axiom : axioms) {
            ...
            if (axiom.getAnnotatedProperty().equals("P375")) {
              ...
              if (!isNull(axiom.getTargetTerminology())
                  && axiom.getTargetTerminology().compareTo(source) == 0) {
                include = true;
              }
      ...
      if (parentConcept != null) {
        System.out.println(
            "\tparentConcept: " + parentConcept.getLabel() + "(" + parentConcept.getCode() + ")");
      }
    }
  }

  public List<String> processRole(
      String code,
      String roleName,
      List<EvsProperty> conceptProperties,
      String property,
    ...
    try {
      List<String> roleTargetCodes =
          sparqlQueryManagerService.getRoleTargets(namedGraph, code, roleName, restURL);
      ...
      for (int i = 0; i < roleTargetCodes.size(); i++) {
        ...
        if (conceptHash.containsKey(targetCode)) {
          ...
          assocConcept =
              sparqlQueryManagerService.getEvsConceptDetailShort(targetCode, namedGraph, restURL);
          assocConcept.setProperties(
              sparqlQueryManagerService.getEvsProperties(targetCode, namedGraph, restURL));
          assocConcept.setAxioms(
              sparqlQueryManagerService.getEvsAxioms(targetCode, namedGraph, restURL));
        ...
        } else if (col_property.compareTo("DEFINITION") == 0
            || col_property.compareTo("P97") == 0) {
          values.addAll(getDefinition(column, conceptAxioms));
        } else if (col_property.compareTo("ALT_DEFINITION") == 0
            || col_property.compareTo("P325") == 0) {
          values.addAll(getDefinition(column, conceptAxioms));
        } else if (col_property.compareTo("term-name") == 0
            || col_property.compareTo("P382") == 0) {
          values.addAll(getFullSynonym(column, conceptAxioms));
        } else if (col_display.compareTo("subsource_code") == 0
            || col_display.compareTo("P385") == 0) {
          values.addAll(getFullSynonym(column, conceptAxioms));
  ...
  // values = processAssociation(associationCode, conceptProperties, property, column, conceptHash,
  // namedGraph, restURL);

  public List<String> processAssociation(
      String code,
    ...
    try {
      List<String> associationTargetCodes =
          sparqlQueryManagerService.getAssociationTargets(
              namedGraph, code, associationName, restURL);
  ...
  public ArrayList<String> getMatchedValuesFromFullSynonyms(
      List<EvsAxiom> axioms,
      String qualifier,
      String termGroup,
      String termSource,
      String sourceCode,
  ...
  public ArrayList<String> getFullSynonym(TemplateColumn column, List<EvsAxiom> axioms) {
    ...
    */
    ...
    if (property.compareTo("property") == 0 && display.compareTo("subsource_code") == 0) {
      values =
          getMatchedValuesFromFullSynonyms(
              axioms, "sourceCode", termGroup, termSource, null, subsource);
    ...
    } else if (display.compareTo("property") == 0
        && (property.compareTo("P382") == 0 || property.compareTo("term-name") == 0)) {
      values =
          getMatchedValuesFromFullSynonyms(
              axioms, "termName", termGroup, termSource, null, subsource);
   ...
   * @param templateColumns TemplateColumns contain template definitions for each column.
   */
  public void processAssociatedConcepts(
      Report reportOutput,
      EvsConcept rootConcept,
      HashMap<String, EvsConcept> conceptHash,
      List<TemplateColumn> templateColumns,
    ...
    if (rootConcept == null) {
      ...
      if (rootConceptCode == null) {
        ...
        List<EvsConcept> associatedConcepts =
            sparqlQueryManagerService.getAssociatedEvsConcepts(
                rootConcept.getCode(), namedGraph, restURL, associationName, sourceOf);
        if (rootConcept != null) {
          log.info(
              "Concept: "
                  + rootConcept.getCode()
        ...
        for (EvsConcept concept : associatedConcepts) {
          ...
          if (conceptHash.containsKey(concept.getCode())) {
            ...
            try {
              concept.setProperties(
                  sparqlQueryManagerService.getEvsProperties(
                      concept.getCode(), namedGraph, restURL));
              concept.setAxioms(
                  sparqlQueryManagerService.getEvsAxioms(concept.getCode(), namedGraph, restURL));
              conceptHash.put(concept.getCode(), concept);
            } catch (Exception ex) {
  ...
  public void processAssociatedConcepts(
      ...
      boolean sourceOf) {
    try {
      List<EvsAssociation> evsAssociations =
          sparqlQueryManagerService.getEvsAssociations(
              namedGraph, restURL, associationName, sourceOf);
      ...
      while (it.hasNext()) {
        ...
        rootConcept.setProperties(
            sparqlQueryManagerService.getEvsProperties(rootConcept.getCode(), namedGraph, restURL));
        rootConcept.setAxioms(
            sparqlQueryManagerService.getEvsAxioms(rootConcept.getCode(), namedGraph, restURL));
  ...
  public Report loadReport(Vector data_vec, char delim) {
    ...
    for (int i = 1; i < data_vec.size(); i++) {
      ...
      for (int j = 0; j < u.size(); j++) {
        ReportColumn column =
            new ReportColumn((String) heading_vec.elementAt(j), (String) u.elementAt(j));
        columns.add(column);
      }
      ...
      pw =
          new PrintWriter(
              new OutputStreamWriter(
                  new FileOutputStream(new File(outputFileText)), StandardCharsets.UTF_8),
              true);
  ...
  public static String getCDISCSubmissionValue(List<EvsAxiom> parentAxioms, List<EvsAxiom> axioms) {
    ...
    List<String> cdisc_pts =
        EVSUtils.getSynonymsWithQualifiers(axioms, termSource, termType, sourceCode, subsourceName);
    ...
    subsourceName = null;
    List<String> nci_abs =
        EVSUtils.getSynonymsWithQualifiers(
            parentAxioms, termSource, termType, sourceCode, subsourceName);
