 ...
 * @version 1.0
 */
public class AsciiToHtmlFormatter extends FileFormatterBase implements FormatterConstant {
  ...
  protected void printContent(TabFormatterInterface out, String textfile, String delimiter)
      throws Exception {
    ...
    if (_specialCases_CDISC) extensible_col = findColumnIndicator(headings, "Extensible");
    ...
    while (true) {
      ...
      for (int i = 0; i < numMissingCells; ++i) v.add(null);
      ...
      if (_specialCases_CDISC && extensible_col >= 0) {
        ...
        if (eValue == null || eValue.trim().length() <= 0) bgColor = " bgColor=\"skyblue\"";
      }
      ...
      for (int col = 0; col < v.size(); col++) {
        ...
        out.writeln_inden1("<td class=\"dataCellText\"" + bgColor + ">" + value + "</td>");
      }
      out.writeln_undent("</tr>");
      row++;
    }
  ...
  private DisplayNCItCodeUrl _displayNCItCodeUrl = DisplayNCItCodeUrl.CurrentWindow;
  ...
  public static void test(String textfile, int[] ncitCodeColumns) {
    try {
      ...
      formatter.setDisplayNCItCodeUrl(DisplayNCItCodeUrl.SeparateSingleWindow);
