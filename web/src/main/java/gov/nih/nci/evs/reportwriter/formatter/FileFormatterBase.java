...
public abstract class FileFormatterBase {
  private static final Logger _logger = LoggerFactory.getLogger(FileFormatterBase.class);

  public abstract Boolean convert(String textfile, String delimiter) throws Exception;

  protected Boolean convert2(String textfile, String toExt, String delimiter) throws Exception {
    int i = textfile.lastIndexOf(".");
    String outfile = textfile.substring(0, i) + "." + toExt;
    return convert(textfile, delimiter, outfile);
  }

  protected abstract Boolean convert(String infile, String delimiter, String outfile)
      throws Exception;
  ...
  protected Vector<String> getColumnHeadings(String textfile, String delimiter) throws Exception {
  ...
  protected Vector<Integer> getColumnMaxChars(String textfile, String delimiter) throws Exception {
    ...
    String line = br.readLine();
    Vector<String> v = parseData(line, delimiter);
    for (int i = 0; i < v.size(); i++) maxChars.add(Integer.valueOf(0));
  ...
  protected Boolean[] findWrappedColumns(String textfile, String delimiter, int maxLength)
      throws Exception {
    Boolean[] a = null;
    BufferedReader br = getBufferReader(textfile);
  ...
  protected int getHeightInPoints(Vector<String> v, boolean adjustHeight, int maxCellWidth) {
    int num_lines = 1;
    if (!adjustHeight) return num_lines;
  ...
  protected int findColumnIndicator(Vector<String> headings, String label) {
    for (int i = 0; i < headings.size(); i++) {
      ...
      if (heading.contains(label)) return i;
    }
    return -1;
  }

  protected int getMaxTokenLength(String heading) {
    if (heading == null || heading.length() == 0) return 0;
    int max = 0;
    String delimiter = " ";
  ...
  public void addNcitCodeColumn(int column) {
    if (!_ncitCodeColumns.contains(column)) _ncitCodeColumns.add(column);
  }

  public void setNcitCodeColumns(int[] columns) {
    if (columns == null) return;
    clearNcitCodeColumns();
    for (int i = 0; i < columns.length; ++i) addNcitCodeColumn(columns[i]);
  }

  public void setNcitUrl(String url) {
    if (url == null || url.trim().length() <= 0) {
      _logger.warn("URL is not set.  Using default NCIt URL: " + DEFAULT_URL);
      url = DEFAULT_URL;
    }
  ...
  protected String getNCItCodeUrl(String code) {
    String ncitCodeUrl =
        _ncitUrl + "ConceptReport.jsp?dictionary=NCI%20Thesaurus" + "&code=" + code;
    return ncitCodeUrl;
  }
}
